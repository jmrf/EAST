    ...
    for i, gpu_id in enumerate(gpus):
        with tf.device('/gpu:%d' % gpu_id):
            with tf.name_scope('model_%d' % gpu_id) as scope:
                ...
                total_loss, model_loss = tower_loss(
        ...
        for step in range(FLAGS.max_steps):
            ...
            if step % 10 == 0:
                ...
                avg_examples_per_second = (
                    10 * FLAGS.batch_size_per_gpu * len(gpus)) / (time.time() - start)
                start = time.time()
                print('Step {:06d}, model loss {:.4f}, '
                      'total loss {:.4f}, {:.2f} seconds/step, '
                      '{:.2f} examples/second'.format(step, ml, tl,
